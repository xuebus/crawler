...
private[crawler] trait StorageManager extends Logging {
  ...
  def getByKey[T: ClassTag](key: String): T = { null.asInstanceOf[T] }
  
  def getByKeyWithException[T: ClassTag](key: String): T = { throw new Exception("Not Implemented") }

  def asynByKey(key: String, f: (HttpResponse) => Unit) = {}

  def listByKey[T: ClassTag](key: String): Seq[T] = { null.asInstanceOf[Seq[T]] }

  def putByKey[T: ClassTag](key: String, entity: T): Boolean = { false }

  def put[T: ClassTag](entity: T): Boolean = { false }

  def delByKey[T: ClassTag](key: String): Boolean = { false }
