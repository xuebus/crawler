 ...
 * @author soledede
 */
private[crawler] class RedisMapTaskQueueImpl private (conf: CrawlerConf) extends ITaskQueue with Logging {
  ...
  override def getHashmapByKeyField[T: ByteStringDeserializer](key: String, fields: Seq[String]): Seq[Option[T]] = {
    redis.getHashmapByKeyField(key, fields)
  }

}

object RedisMapTaskQueueImpl {
  var redisMap: RedisMapTaskQueueImpl = null
  def apply(conf: CrawlerConf): RedisMapTaskQueueImpl = {
    if (redisMap == null) redisMap = new RedisMapTaskQueueImpl(conf)
    redisMap
  }
}
